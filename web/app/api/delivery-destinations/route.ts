...
export async function GET(request: Request) {
  try {
    const access = await ensureMenuAccess(request, "deliveryDestinations", 1);
    if (!access.ok) {
    ...
    if (error) return serverError("納品先の取得に失敗しました。", error.message);
    return NextResponse.json({ ok: true, data: data ?? [] }, { status: 200 });
  } catch (error) {
    const details = error instanceof Error ? error.message : "Unknown error";
    return serverError("納品先の取得に失敗しました。", details);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!destinationName) return badRequest("destinationNameは必須です。");
    const supabase = createSupabaseAdminClient();
    const { data, error } = await supabase
      .from("delivery_destinations")
      .insert({
      ...
      .select("*")
      .single();
    if (error) return serverError("納品先の作成に失敗しました。", error.message);
    return NextResponse.json({ ok: true, data }, { status: 201 });
  } catch (error) {
    const details = error instanceof Error ? error.message : "Unknown error";
    return serverError("納品先の作成に失敗しました。", details);
  }
}
